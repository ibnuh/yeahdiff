<script lang="ts">
	...
	let { paneId, paneIndex, stats }: Props = $props();
...
>
	<div class="flex items-center gap-2 min-w-0">
		...
		{/if}
		<select
			class="px-1 py-0.5 text-xs rounded bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-400 border-none outline-none cursor-pointer max-w-[9rem]"
			value={pane?.manualLanguage ?? 'auto'}
			onchange={handleLanguageChange}
			title="Select language"
			aria-label="Language for pane {paneIndex + 1}"
		>
			<option value="auto">
				{pane?.manualLanguage
					? 'Auto'
					: pane?.detectedLanguage
						? `Auto (${pane.detectedLanguage})`
						: 'Auto'}
			</option>
		...
		{#if isBase}
			<span
				class="px-1.5 py-0.5 text-xs rounded bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300 shrink-0"
			>
				base
			</span>
		{/if}

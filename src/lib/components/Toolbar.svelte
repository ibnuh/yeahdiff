<script lang="ts">
	...
	async function handleShare() {
		...
		if (result.tooLong) {
			toast.info('Link copied, but it may be too long for some browsers. Prefer Export Session for large content.');
		} else {
	...
	async function handleFileSelect(e: Event) {
		...
		if (file) {
			...
			if (result.ok) {
				...
				toast.error(result.error ?? 'Failed to import session');
			}
			closeMenu();
		}
		target.value = '';
...
>
	...
	<div class="flex items-center gap-1.5 sm:gap-2 md:gap-3 shrink-0">
		...
		<button
			...
			aria-label="Add pane"
		>
			<svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
				><path d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" /></svg
			>
			<span class="hidden lg:inline">Add Pane</span>
		</button>

		<div class="hidden sm:block mx-1 h-5 w-px bg-gray-300 dark:bg-gray-600" aria-hidden="true"></div>

		<div
			class="flex items-center rounded-md overflow-hidden border border-gray-300 dark:border-gray-600 shrink-0"
			role="group"
		...
		>
			<button
				...
				title="Compare all panes against a base pane"
				aria-pressed={settings.diffMode === 'base'}
			>
				<svg
					class="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-3.5 md:h-3.5 shrink-0"
					viewBox="0 0 20 20"
					fill="currentColor"
					aria-hidden="true"
					><path
			...
			<button
				...
				aria-pressed={settings.diffMode === 'adjacent'}
			>
				<svg
					class="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-3.5 md:h-3.5 shrink-0"
					viewBox="0 0 20 20"
	...
	<div class="hidden sm:flex items-center gap-0.5 sm:gap-1 shrink-0">
		...
		<button
			...
			title="Align matching lines across all panes"
			aria-pressed={settings.alignedDiff}
		>
			<svg class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
				><path
					fill-rule="evenodd"
		...
		>
			<svg class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
				><path
					fill-rule="evenodd"
					d="M4 2a1 1 0 00-1 1v14a1 1 0 102 0V3a1 1 0 00-1-1zm12 0a1 1 0 00-1 1v14a1 1 0 102 0V3a1 1 0 00-1-1zM7.293 6.293a1 1 0 011.414 0L10 7.586l1.293-1.293a1 1 0 111.414 1.414l-2 2a1 1 0 01-1.414 0l-2-2a1 1 0 010-1.414zm0 6a1 1 0 011.414 0L10 13.586l1.293-1.293a1 1 0 111.414 1.414l-2 2a1 1 0 01-1.414 0l-2-2a1 1 0 010-1.414z"
					clip-rule="evenodd"
				/></svg
			>
		...
		>
			<svg class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
				><path
					fill-rule="evenodd"
					d="M3 5a1 1 0 011-1h12a1 1 0 010 2H4a1 1 0 01-1-1zm0 10a1 1 0 011-1h4a1 1 0 010 2H4a1 1 0 01-1-1zm0-5a1 1 0 011-1h9a3 3 0 010 6h-1.586l.293.293a1 1 0 01-1.414 1.414l-2-2a1 1 0 010-1.414l2-2a1 1 0 011.414 1.414L12.414 13H13a1 1 0 100-2H4a1 1 0 01-1-1z"
					clip-rule="evenodd"
				/></svg
			>
		...
		>
			...
			{:else}
				<svg class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
					><path
						fill-rule="evenodd"
						d="M3 4a1 1 0 011-1h4a1 1 0 010 2H6.414l2.293 2.293a1 1 0 01-1.414 1.414L5 6.414V8a1 1 0 01-2 0V4zm9 1a1 1 0 010-2h4a1 1 0 011 1v4a1 1 0 01-2 0V6.414l-2.293 2.293a1 1 0 11-1.414-1.414L13.586 5H12zM5.293 12.293a1 1 0 011.414 0L9 14.586V13a1 1 0 112 0v4a1 1 0 01-1 1H6a1 1 0 110-2h1.586l-2.293-2.293a1 1 0 010-1.414z"
						clip-rule="evenodd"
					/></svg
				>
			{/if}
	...
	<div class="flex md:hidden items-center gap-2">
		<button
			...
			aria-label={settings.mobileLayout === 'compare'
				? 'Switch to stacked view'
				: 'Switch to comparison view'}
			title={settings.mobileLayout === 'compare'
				? 'Switch to stacked view'
				: 'Switch to comparison view'}
		>
			...
			{:else}
				<svg class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
					><path
						fill-rule="evenodd"
						d="M3 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 8a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0-4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 8a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z"
						clip-rule="evenodd"
					/></svg
				>
			{/if}
...
{#if mobileMenuOpen}
	...
	<div
		class="absolute right-0 top-0 h-full w-64 bg-white dark:bg-gray-900 shadow-lg overflow-y-auto z-50"
		role="dialog"
		aria-modal="true"
		aria-label="Settings"
	>
		<div class="p-4 space-y-3">
			...
			<button
				class="w-full px-3 py-3 rounded-lg transition-colors flex items-center gap-2 min-h-[48px] bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300"
				onclick={() => {
					handleShare();
					closeMenu();
				}}

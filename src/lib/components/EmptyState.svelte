...
<div class="flex flex-col items-center justify-center h-full p-8 text-center">
	<div class="max-w-md space-y-6">
		...
		<div class="space-y-3">
			<p class="text-sm font-medium text-gray-500 dark:text-gray-500">Try an example:</p>
			<div class="space-y-2">
				{#each examples as example}
					<button
						type="button"
						class="w-full text-left p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
						onclick={example.load}
					>
						<div class="font-medium text-gray-800 dark:text-gray-200">{example.name}</div>
						<div class="text-sm text-gray-500 dark:text-gray-500">{example.description}</div>
					</button>
				{/each}
			</div>
		</div>

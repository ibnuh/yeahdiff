<script lang="ts">
	...
	let searchResults = $state<
		Array<{ paneIndex: number; lineNumber: number; lineContent: string }>
	>([]);
	...
	function performSearch() {
		...
		if (results.length > 0) {
			jumpToResult(0, false);
		}
	}

	function jumpToResult(index: number, closeAfter = false) {
		if (index < 0 || index >= searchResults.length) return;

		currentResultIndex = index;
		const result = searchResults[index];

		// On mobile stack mode, switch active pane tab
		settings.setBaseIndex(result.paneIndex);
		navigation.jumpTo(result.paneIndex, result.lineNumber);
	...
	function nextResult() {
		...
		jumpToResult((currentResultIndex + 1) % searchResults.length);
	}

	function prevResult() {
		if (searchResults.length === 0) return;
		jumpToResult((currentResultIndex - 1 + searchResults.length) % searchResults.length);
	}
	...
	$effect(() => {
		if (!isOpen) return;
		// Re-run when query or pane contents change
		void searchQuery;
...
{#if isOpen}
	...
	<div
		...
		role="dialog"
		aria-modal="true"
		aria-label="Search across panes"
	>
		<div
		...
		>
				...
				<div class="max-h-80 overflow-y-auto" role="listbox" aria-label="Search results">
					{#each searchResults as result, index}
						<button
							...
							ondblclick={() => jumpToResult(index, true)}
							role="option"
							aria-selected={index === currentResultIndex}
						>
							<div class="flex items-center gap-2 text-sm">
								<span
									class="text-xs font-medium text-gray-500 bg-gray-100 dark:bg-gray-700 dark:text-gray-400 px-1.5 py-0.5 rounded shrink-0"
								>
			...
			<div
				class="p-2 border-t border-gray-200 dark:border-gray-700 text-xs text-gray-400 flex items-center justify-between"
			>
				<span>↑↓ navigate · Enter next · Esc close</span>
			</div>
		</div>
	</div>
{/if}
